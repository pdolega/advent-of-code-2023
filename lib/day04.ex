defmodule Day04 do
  def part1(input) do
    Enum.map(parse_input(input), fn {card_nums, my_nums} ->
  ...
  def part2(input) do
    scratchcards = parse_input(input) |> Enum.with_index(1)
    won_cards = Enum.map(scratchcards, fn {_, index} -> {index, 0} end) |> Enum.into(%{})

    new_scratchcards = Enum.reduce(scratchcards, won_cards, fn {scratchcard, index}, won_scratchcards ->
    ...
    new_scratchcard_sum = Map.values(new_scratchcards) |> Enum.sum()
  ...
  defp parse_input(input) do
    ...
    |> Enum.map(fn line ->
      ...
      nums_left = Regex.scan(nums_regex, nums_on_card) |> Enum.map(fn [_, num] -> String.to_integer(num) end)
      nums_right = Regex.scan(nums_regex, my_nums) |> Enum.map(fn [_, num] -> String.to_integer(num) end)
